...
<head>
    ...
    <title>Exception - {{ $error_code ?? 500 }} | The Framework</title>
            ...
            <div class="space-y-6 animate-fade-in">
                ...
                <div class="text-center space-y-4">
                    <div class="inline-flex items-center gap-3 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-full">
                        ...
                        <span class="text-amber-400 font-medium">HTTP {{ $error_code ?? 500 }} - {{ $error_code_text ?? 'Internal Server Error' }}</span>
                    </div>
                    <h1 class="text-6xl sm:text-7xl font-bold bg-gradient-to-r from-amber-400 via-orange-500 to-red-500 bg-clip-text text-transparent">
                        {{ $class }}
                    </h1>
...
@endforeach</code></pre>
                ...
                <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden">
                    <div class="bg-slate-900/50 px-5 py-3 border-b border-slate-700/50 flex items-center gap-2">
                        ...
                        <span class="text-sm font-medium text-slate-300">Stack Trace</span>
                    </div>
                    <div class="p-4 space-y-3 max-h-96 overflow-y-auto">
                        ...
                        <div class="bg-slate-900/50 border border-slate-700/30 rounded-lg p-4">
                            <div class="flex items-start gap-3">
                                <span class="flex-shrink-0 w-8 h-8 bg-cyan-500/20 text-cyan-400 rounded-full flex items-center justify-center text-xs font-bold">
                                    #{{ $index }}
                                </span>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mt-2 text-xs text-slate-500">
                                        <span class="text-slate-600">Args:</span>
                                        <span class="text-slate-400 ml-1">{{ implode(', ', array_slice($traceItem['args'], 0, 3)) }}{{ count($traceItem['args']) > 3 ? '...' : '' }}</span>
                                    </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden">
                        ...
                        <div class="p-4 space-y-3 text-sm">
                            ...
                            <div>
                                ...
                                <pre class="text-xs text-slate-400 mt-1 overflow-x-auto">{{ json_encode($request_info['query'], JSON_PRETTY_PRINT) }}</pre>
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-red-500/10 border border-red-500/20 rounded-xl p-5">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="text-sm font-medium text-red-400">Previous Exception</span>
                    </div>
                    ...
                    <p class="text-xs text-slate-500 font-mono">{{ $previous->getFile() }}:{{ $previous->getLine() }}</p>
                </div>
                @endif
