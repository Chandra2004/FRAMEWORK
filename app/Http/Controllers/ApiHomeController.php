...
class ApiHomeController extends Controller
{
    ...
    public function CreateUser()
    {
        try {
            ...
            if (is_array($resultUser) && UploadHandler::isError($resultUser)) {
                return json(['status' => 'error', 'message' => UploadHandler::getErrorMessage($resultUser)], 400);
            }

            if (!$resultUser) {
                return json(['status' => 'error', 'message' => 'Failed to create user in database'], 500);
            }

            if (is_string($resultUser) && array_key_exists($resultUser, self::CREATE_ERRORS)) {
                return json(['status' => 'error', 'message' => self::CREATE_ERRORS[$resultUser]], 400);
            }
            ...
            return json(['status' => 'error', 'message' => 'Server Error: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function UpdateUser($uid)
    {
        try {
            ...
            if (is_string($result) && array_key_exists($result, self::CREATE_ERRORS)) {
                return json(['status' => 'error', 'message' => self::CREATE_ERRORS[$result]], 400);
            }

            if (!$result) {
                return json(['status' => 'error', 'message' => 'Failed to update user'], 500);
            }

            // Ambil data user terbaru
            $updatedUser = $this->userService->getUserByEmail($request->input('email')); 
    ...
    public function DeleteUser($uid)
    {
        try {
            ...
            return json(['status' => 'success', 'message' => 'User successfully deleted']);
        } catch (Exception $e) {
            return json(['status' => 'error', 'message' => $e->getMessage()], 500);
        }
    }
